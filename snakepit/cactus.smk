...
#TODO: cactus does motify the seqfile
#TODO: which step modifies it? mg or graphmap? Which input needs it?
...
rule cactus_minigraph:
    input:
        seqfile = rules.cactus_copy_seqfile.output['seqfile']
        #seqfile = config['seq_file']
    output:
    ...
    shell: '''
cactus-minigraph $TMPDIR/minigraph {input.seqfile} {output.gfa} --reference {config[reference_ID]}
'''
...
rule cactus_graphmap:
    ...
    container: config["cactus_image"]
    shell: '''
cactus-graphmap $TMPDIR/graphmap {input.seqfile} {input.gfa} {output.paf} --outputFasta {output.fasta} --reference {config[reference_ID]}
'''
...
checkpoint cactus_split:
    ...
    output:
        ...
        contig_sizes = 'cactus/{graph}/split/contig_sizes.tsv',
    ...
    container: config["cactus_image"]
    shell: '''
cactus-graphmap-split $TMPDIR/split {input.seqfile} {input.gfa} {input.paf} --outDir {output.split_dir} --reference {config[reference_ID]}
'''
...
rule cactus_align:
    ...
    container: config["cactus_image"]
    shell: '''
cactus-align $TMPDIR/align {input.seqfile} {input.paf} {output.hal} --pangenome --reference {config[reference_ID]} --outVG
'''

def gather_alignments(wildcards):
    with checkpoints.cactus_split.get(**wildcards).output.chromfile.open() as f:
        return [f'cactus/{{graph}}/align/{row.split("\t")[0]}.vg' for row in f]
...
rule cactus_join:
    ...
    output:
        multiext('cactus/{graph}/{graph}', hal='.full.hal', gfa='.gfa.gz', vcf='.vcf.gz', vcf_index='.vcf.gz.tbi', gbz='.gbz', raw_vcf='.raw.vcf.gz', raw_vcf_index='.raw.vcf.gz.tbi', stats='.stats.tgz')
    params:
    ...
    container: config["cactus_image"]
    shell: '''
cactus-graphmap-join $TMPDIR/join --vg {input.vg} --outDir {params.out_dir} --outName {wildcards.graph} --reference {config[reference_ID]} --vcf --giraffe full clip
'''