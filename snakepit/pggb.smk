...
rule wfmash:
    input:
        ...
        mapping=lambda wildcards: (
            "analyses/pggb/{graph}/p{p}_s{segment_length}/{chromosome}.mapping{chunk}.paf"
            if wildcards.mode == "alignment"
            else []
        ),
    output:
    ...
    params:
        block_length=lambda wildcards: int(wildcards.segment_length) * 3,
        mapping=lambda wildcards, input: (
            f"--align-paf {input.mapping}"
            if wildcards.mode == "alignment"
            else "--approx-mapping"
        ),
...
rule split_approx_mappings_in_chunks:
    input:
        mapping=expand(
            rules.wfmash.output["paf"], mode="mapping", chunk="", allow_missing=True
        ),
    output:
        paf=expand(
            "analyses/pggb/{graph}/p{p}_s{segment_length}/{chromosome}.mapping.{chunk}.paf",
            chunk=range(config.get("wfmash_chunks", 1)),
            allow_missing=True,
        ),
    params:
    ...
    run:
        ...
        with open(input["mapping"][0]) as fin:
            for rank, line in enumerate(fin):
                ...
                weights[idx] += max(
                    int(query_end) - int(query_start),
                    int(target_end) - int(target_start),
                ) * (100 - float(estimated_identity.split("id:f:")[1]))
...
rule seqwish:
    input:
        fasta=rules.panSN_gather.output["fasta"],
        alignment=(
            expand(rules.wfmash_concat.output["paf"], allow_missing=True)
            if config.get("wfmash_chunks", 1) > 1
            else expand(
                rules.wfmash.output["paf"],
                mode="alignment",
                chunk="",
...
rule smoothxg:
    ...
    params:
        ...
        POA_pad_depth=lambda wildcards, input: 100
        * sum(1 for _ in open(input.fasta[1])),
...
rule vg_path_normalise:
    ...
    params:
        reference=lambda wildcards, input: open(input.fasta[1])
        .readline()
        .rstrip()
        .split("\t")[0],
    threads: 4
...
rule odgi_unchop:
    input:
        gfa=(
            rules.vg_path_normalise.output["gfa"]
            if config.get("Normalise_path", False)
            else rules.GFAffix.output["gfa"]
        ),
    output:
...
rule odgi_layout:
    ...
    output:
        layout=multiext(
            "analyses/pggb/{graph}/p{p}_s{segment_length}/{chromosome}.k{k}.POA{POA}.unchop.lay",
            "",
            ".tsv",
        ),
    threads: 6
...
rule odgi_draw:
    ...
    output:
        image=multiext(
            "analyses/pggb/{graph}/p{p}_s{segment_length}/{chromosome}.k{k}.POA{POA}.unchop.{draw}",
            ".png",
            ".svg",
        ),
    params:
