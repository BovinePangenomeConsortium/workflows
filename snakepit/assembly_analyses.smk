config["reference"] = (
    "/cluster/work/pausch/inputs/ref/BTA/UCD2.0/GCA_002263795.4_ARS-UCD2.0_genomic.fa"
)
...
def get_samples():
    ...
    return metadata.get_column(  # .filter(pl.col('Species')=='Bos taurus')
        "sample"
    ).to_list()
...
rule all:
    input:
        expand(
            "analyses/scaffolding/{reference}/{sample}",
